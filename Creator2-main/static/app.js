...
async function startGenerating(forceDemo, useGemini) {
  ...
  // Cosmetic progress: step through the pipeline stages while the request runs.
  const steps = [...document.querySelectorAll("#wait-steps li")];
  let i = 0;
  const paint = () => steps.forEach((li, j) => { li.classList.toggle("done", j < i); li.classList.toggle("active", j === i); });
  paint();
  const timer = setInterval(() => { if (i < steps.length - 1) { i++; paint(); } }, 900);
  const ok = await runAnalysis(forceDemo, useGemini);
  clearInterval(timer);
  i = steps.length; paint();
  await sleep(350);
...
function renderOverview(ins, results, brief) {
  ...
  countUp($("tab-overview"));
}

// Animate numeric [data-count] elements from 0 to their value (visual only).
...
function renderHero(bp) {
  return `<div class="hero-card">
    <div class="hero-top">
      ...
      <div class="score-ring" style="--p:${Number(bp.total) || 0}"><b data-count="${bp.total}">${bp.total}</b><span>/100</span></div>
    </div>
...
function renderRankCard(c, sources) {
  ...
  return `<div class="rank-card ${c.rank <= 3 ? "top top-" + c.rank : ""}">
    <div class="rank-head">
      ...
      <div class="score-ring sm" style="--p:${Number(c.total) || 0}"><b data-count="${c.total}">${c.total}</b></div>
    </div>
...
function drawBudgetChart(rows) {
  ...
  google.charts.setOnLoadCallback(() => {
    ...
    chart.draw(data, {
      ...
      colors: ["#7C4DFF"], backgroundColor: "transparent",
      legend: { position: "top", textStyle: { color: "#5F6368" } },
      hAxis: { textStyle: { color: "#5F6368" } },
      vAxes: {
        0: { title: "Cost (USD)", format: "$#,###", titleTextStyle: { color: "#7C4DFF", italic: false }, textStyle: { color: "#5F6368" }, gridlines: { color: "#DADCE0" } },
...
document.querySelectorAll("#results-tabs .tab").forEach((t) => {
  t.addEventListener("click", () => {
    ...
    countUp($("tab-" + t.dataset.tab));
  });
});

// Cursor spotlight on cards (sets --mx/--my for the CSS radial highlight)
document.addEventListener("pointermove", (e) => {
  const card = e.target.closest && e.target.closest(".hero-card, .rank-card, .bcard, .member-row, .campaign-summary");
  if (!card) return;
